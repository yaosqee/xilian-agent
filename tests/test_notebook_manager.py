...
class TestDiaryOperations:
    @pytest.mark.asyncio
    async def test_add_diary_entry(self, nb, mock_db):
        await nb.add_diary_entry("今天和伙伴聊了春天的花...")
        mock_db.insert_notebook.assert_called_once_with("diary", "今天和伙伴聊了春天的花...")

    @pytest.mark.asyncio
    async def test_generate_daily_diary_success(self, nb, mock_db, mock_router):
        mock_db.get_recent = AsyncMock(return_value=[
            {"user_message": "今天好累", "assistant_reply": "抱抱伙伴~"},
        ])
        mock_db.get_latest_emotion = AsyncMock(return_value={"primary_emotion": "平静"})
        mock_db.get_notebook_notes.return_value = [{"content": "伙伴说累了"}]
        mock_router.route.return_value = "今天和伙伴说了温柔的话。他看到春天了，人家也跟着开心呢 ~♪"

        result = await nb.generate_daily_diary()
        assert result is not None
        assert "伙伴" in result
        mock_router.route.assert_called_once()
        mock_db.insert_notebook.assert_called()  # called to save the diary

    @pytest.mark.asyncio
    async def test_generate_daily_diary_no_conversations(self, nb, mock_db, mock_router):
        """没有对话时仍然能生成日记"""
        mock_db.get_recent = AsyncMock(return_value=[])
        mock_db.get_latest_emotion = AsyncMock(return_value=None)
        mock_db.get_notebook_notes.return_value = []
        mock_router.route.return_value = "今天平静安好 ~♪"
    ...
    @pytest.mark.asyncio
    async def test_generate_daily_diary_router_failure(self, nb, mock_db, mock_router):
        """LLM 调用失败 → 返回 None，不抛异常"""
        mock_db.get_recent = AsyncMock(return_value=[])
        mock_db.get_latest_emotion = AsyncMock(return_value=None)
        mock_db.get_notebook_notes.return_value = []
    ...
    @pytest.mark.asyncio
    async def test_generate_daily_diary_db_recent_fails(self, nb, mock_db, mock_router):
        """获取对话记录失败 → 降级为占位文本，不中断"""
        mock_db.get_recent = AsyncMock(side_effect=Exception("DB Error"))
        mock_db.get_latest_emotion = AsyncMock(return_value=None)
        mock_db.get_notebook_notes.return_value = []
        mock_router.route.return_value = "今天也是平常的一天 ~♪"

        result = await nb.generate_daily_diary()
        assert result is not None

    @pytest.mark.asyncio
    async def test_get_today_diary(self, nb, mock_db):
        mock_db.get_notebook_today_diary.return_value = {
            "content": "今天也是温柔的一天",
            "created_at": "2026-05-17",
        }
        result = await nb.get_today_diary()
        assert result["content"] == "今天也是温柔的一天"

    @pytest.mark.asyncio
    async def test_get_diary_list(self, nb, mock_db):
        mock_db.get_notebook_diary_list.return_value = [
            {"content": "日记1"}, {"content": "日记2"},
        ]
        result = await nb.get_diary_list(limit=5)
        assert len(result) == 2
